...
const string MEASUREMENT_FILE = @"./measurements.txt";
...
if (args.Length != 1)
{
    Console.Error.WriteLine("Usage: 1brc_gen <number of records to create>");
    Environment.Exit(1);
}

if (!int.TryParse(args[0], out var size))
{
    Console.Error.WriteLine("Invalid value for <number of records to create>");
    Console.Error.WriteLine("Usage: 1brc_gen <number of records to create>");
    Environment.Exit(1);
}

using var sw = new StreamWriter(MEASUREMENT_FILE);
...
for (int i = 0; i < size; i++)
{
    if (i > 0 && i % 50000000 == 0)
    {
        Console.WriteLine($"Wrote {i} measurements in {stopwatch.ElapsedMilliseconds} ms");
    }

    var weatherStation = stations[Random.Shared.Next(stations.Length)];
    sw.WriteLine(weatherStation);
}

stopwatch.Stop();

Console.WriteLine(
    $"Created file with {size} measurements in {stopwatch.ElapsedMilliseconds} ms"
);

record WeatherStation(string Id, double MeanTemperature)
{
    public double Measurement => MeanTemperature + (Random.Shared.NextDouble() * 20) - 10;
